...
library(raster)
library(clpAPI, lib.loc="/gpfs/home/pbarbieri/R/x86_64-pc-linux-gnu-library/3.5")
#library(Rglpk)
library(ROI.plugin.clp, lib.loc="/gpfs/home/pbarbieri/R/x86_64-pc-linux-gnu-library/3.5")
...
setwd("/gpfs/home/pbarbieri/global/Version_I_first_inclusion_of_land_use_change_rules/")
getwd()
library(unixtools)
...
#.libPaths(c("/gpfs/home/pbarbieri/R/x86_64-pc-linux-gnu-library/3.5"))
...
# -------------------------------------------- 1 Binary coefficients to activate/disactivate variables and global scenario coefficients -------------------------------------
		...
		global_production_share_coefficient_org <- 1 - global_production_share_coefficient_conv
			...
			K_waste_water <- 1  # 0-1 Desactivate/Activate the use of conventional surplus manure. Wastewater as already been accounted for losses in the conventional-submodel
...
#     *************************** ATT!! Correction of Organic Area according to global organic share **************************
	...
	foreach (i = 1:61, .packages="raster") %do% {
        matrix_yield_max_vec[,,i] <- as.vector(raster(paste0("Conventional yield files/", crop_info$CropName[i], "_YieldPerHectare.tif"))*crop_info$dry_matter[i])                  # Load organic max yields with modified Ponision coefficents and tranform them in DM
        matrix_area_vec[,,i] <- as.vector(raster(paste0("Conventional harvested areas/", crop_info$CropName[i], "_HarvestedAreaHectares.tif"))* global_production_share_coefficient_org)  # Load organic areas in ha
	}
...
total_N_manure <- raster("Script_generated_files/total_N_manure_correct_list_countries.tif")
	...
	leaching_areas_mask <- raster("/gpfs/home/pbarbieri/global/Version_I_first_inclusion_of_land_use_change_rules/Climatic and irrigation data/mask_area_leaching_final_irrigation_above_005.tif") # Load leaching areas. The areas where leaching occurs were estimated according to the IPCC procedure. See "Estimation_areas_of_N_leaching.R" in /Pietro/R Analysis/ Global convetion.../ Estimation areas of.../R_code
  ...
  N_chemical_fertilisers <- array(dim=c(1,9331200))
	N_chemical_fertilisers[,] <- as.vector(merge(raster("Script_generated_files/total_N_fert.tif"), reclassify(raster("global/Version_I_first_inclusion_of_land_use_change_rules/Script_generated_files/total_N_manure_correct_list_countries.tif")>0, cbind(1,0))))  #total_N_fert.tif   # Load total applied chemical fertilisers (non crop specific!! the model can reassign N to other crops!)
...
(+(N_conventional_manure[i,j]*K_conv_manure_surplus*global_production_share_coefficient_conv)+(N_conventional_waste_water[i,j]*K_waste_water)+(N_organic_manure[i,j])+(N_chemical_fertilisers[i,j])))  # N available for fertilisers
...
obj[c(1, 12, 18, 21, 23, 27, 30, 33, 58, 44, 53, 54, 16)] <- 0.0000001                                           # set fodder crops to zero AND cotton/rubber/tea/tobacco
...
for (i in 1:61) {
    raster_3 <- raster(matrix(matrix_final_N_assigned_to_crop[,,1], nrow=2160, byrow=TRUE), xmn=-180, xmx=180, ymn=-90, ymx=90, crs = crs)
